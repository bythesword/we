...
scene.addUserDefine({
  call: function (scope: any): any {
    ...
    scope.copyTextureToTexture(scene.stages["World"]!.GBuffers[scene.defaultCameraActor.id.toString()]["color"], colorTexture, { width: scope.canvas.width, height: scope.canvas.height });
    return true;
  },
  name: "copy",
  state: true
