...
fn fs(@builtin(position) coord: vec4f) -> ST_GBuffer_depthAndID {
    ...
    // var index : u32;
    var maxIndex: u32 = 0;
    var maxValue = depths[0];
    if reversedZ {
        for (var i: u32 = 0; i < 4; i = i + 1) {
            if depths[i] < maxValue {
                maxValue = depths[i];
                maxIndex = i;
            }
        }
    ...
    //output.color = vec4f(f32( id2+1)/30., 0,0,1);//ok
    // output.color = vec4f(depths[maxIndex], depth2, depth2, 1);

    output.depth = depths[maxIndex];
