...
vsOutput.position = matrix_z * MVP * entity.MatrixWorld[instanceIndex] * vec4f(position, 1.0);          //vsOutput.position = matrix_z * projectionMatrix * viewMatrix * modelMatrix * entity.MatrixWorld[instanceIndex] * vec4f(position, 1.0);
vsOutput.uv = uv;
...
vsOutput.entityID = instanceIndex + entity_id + stage_id;           //vsOutput.entityID = entity.entity_id;
